...
**啟動順序很重要**：`frontend` 在 Server Components 渲染頁面時會直接打 `backend`，`backend` 沒起來的話首頁會直接噴錯；`rwa-agent` 沒起來時 `/agent` 的聊天會顯示「無法連線到 rwa-agent」，但不影響其他頁面。
...
```bash
# frontend 已經在跑（npm run dev, port 3000）
...
- **領取分紅**：還是純後台模擬（呼叫 `backend` 的 mock 邏輯），不是真的鏈上合約——只有「購買/贊助付款」這個方向做到真實 Devnet 轉帳。年度結算、農夫買回、專案狀態切換這幾個後台管理動作已經整個移除（前端跟 `backend` 都沒有了），不是隱藏起來而已
...
lib/
  ...
  format.ts                 金額／日期格式化，含 TWD↔USD 換算
```
